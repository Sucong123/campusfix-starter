...
## 记录4（日期：2026-09-12，实验：第4次）

- **我原来的目标**：
  学会分支的完整生命周期（创建 → 切换 → 提交 → 合并 → 删除），亲眼看到"分叉与合流"在提交图里长什么样；
  按老师要求用 Fork 观察这些变化。

- **我提供给AI的信息（已脱敏）**：
  第4次课实验指导书的本地路径（`D:\下载\02-实验指导书-学生版 (7).md`）、
  我的学号（用于分支名 `feature-20232032`）、Fork 界面的截图。
  **没有**提供任何密码、Token、私钥、服务器地址或真实个人信息。

- **提示词摘要**：
  1. 「老师让我们在 Fork 上看这次实验。」（贴了指导书路径）
  2. 「我看不到橙色线。」
  3. 「你帮我做一下。」/「你都帮我做。」

- **AI建议摘要**：
  1. 分支练习用命令行做，用 Fork 看结果 —— 因为分支是 Git 的核心操作，但图形界面看分叉比命令行直观。
  2. 提示第八步 `git commit` 不带 `-m` 会打开编辑器（Vim），初学者容易卡死，建议改用 `git commit -m "..."`。
  3. 把"改同一行 → 冲突"的练习简化成"改不同行 → 无冲突合并"，但实际操作中仍然撞上了冲突。

- **我采纳/拒绝的部分及理由**：
  - **采纳**：Fork 用来看、命令行用来做。—— 两边各取所长。
  - **采纳**：`git commit -m` 代替裸 `git commit`。—— 避开了编辑器陷阱。
  - **AI 说错的地方（重要）**：
    a) AI 说"建完分支按 F5 就能看到图上分叉"。**实际不会** —— 我建了 `feature-20232032`
       并切过去之后，提交图仍然是一条直线。因为**分支上还没有任何新提交**，
       `feature-` 和 `main` 指向的是同一条提交，自然画不出分叉。
       必须**在这个分支上做出一条新提交**，图上才会分叉。我实际验证：`git log` 显示
       `4a125de (HEAD -> feature-20232032, main)` —— 括号里两个名字，证明它们在同一位置。
    b) AI 说"两个分支改不同的地方，所以不会冲突"。**实际冲突了**（`CONFLICT (content)`）。
       原因：README 的标题**只有一行**，A、B 两边改的都是**这一行** —— 只要改同一行，Git 就
       无法自动合并。AI 的"改不同地方"在概念上没错，但没有意识到标题只有一行这个事实。
  - **我拒绝的部分**：AI 提议"替你完成冲突解决"。我要求它做完机械部分（分支创建、合并），
    但冲突的解决过程我要求它讲清楚，我核对结果。

- **我实际执行的操作**：
  ```powershell
  git branch feature-20232032
  git switch feature-20232032
  # 改 templates/index.html 标题并提交
  git commit -m "feat: 首页标题改为小组版"

  git switch main                 # ④ 观察：main 上看不到那条提交
  git merge feature-20232032      # ⑤ Fast-forward（快进）
  git branch -d feature-20232032  # ⑥ 删除分支
  ```
  之后又做了"真正的合并"演示：`feature-a` 和 `feature-b` 各改 README 标题，
  合并 `feature-b` 时冲突，在 VS Code 里解决后 `git add` + `git commit`。

- **验证命令/结果**：
  - `git switch main` 后 `git log --oneline` → **看不到** `127e165 feat: 首页标题改为小组版`
    （证明分支之间互相隔离，`merge` 之前彼此的提交不可见）。
  - `git merge feature-20232032` → `Fast-forward`。**这不是指导书预期的那种分叉合并** ——
    因为只有分支单方面往前走了，`main` 自己一步没动，Git 直接把指针挪过去就行。
  - 冲突现场（README.md 第 1-5 行）：
    ```text
    <<<<<<< HEAD
    # CampusFix 校园报修工单系统 — 起始项目（A组版）
    =======
    # CampusFix 校园报修工单系统 — 起始项目（B组版）
    >>>>>>> feature-b
    ```
  - 解决后 `Select-String -Path README.md -Pattern "^<<<<<<<|^=======|^>>>>>>>"` → 无残留标记。
  - 最终 `git log --oneline --graph`：
    ```text
    *   8cd3d77 merge: 合并 feature-b，README标题融合A、B两组版本
    |\
    | * 86b0101 docs: B组修改README标题
    * | 3dd80dd docs: A组修改README标题
    |/
    * 127e165 feat: 首页标题改为小组版
    ```
  - `git branch` → 只剩 `main`；`git status` → 干净。

- **我现在能独立解释的内容**：
  1. **为什么切回 main 后看不到分支上的提交？**
     因为分支是两条**独立的提交线**：`main` 的指针停在分叉点，`feature-` 的指针往前走了一步。
     `git log` 默认只显示**当前分支能追溯到的**提交，所以 main 上看不到 feature 的提交。
     只有 `git merge` 才会把两条线接起来。
  2. **为什么这次合并是 Fast-forward，指导书里却是分叉合并？**
     关键看**分叉之后两边有没有各自前进**：
     - 只有一边动了 → main 无事可做，指针直接挪过去 = Fast-forward（历史保持直线）
     - 两边都动了 → 必须造一个"合并提交"把两条线接上 = 真正的合并（图上出现 `|\` 和 `|/`）
  3. **冲突是怎么产生的？**
     两个分支改了**同一个文件的同一行**。Git 能自动合并"你改 A 行、我改 B 行"，
     但同一行改成了两个不同的样子时，Git 无法判断该保留哪个 —— 于是把两个版本都塞进文件，
     用 `<<<<<<<` / `=======` / `>>>>>>>` 三行标记圈出来，交给人决定。
  4. **我为什么选择"融合"而不是二选一？**
     A、B 两版都是有效信息（标题本身 + 各自的组标识），丢掉任何一个都是信息损失。
     融合成 `（A组版·B组版）` 两边都保留，也符合指导书给的建议写法。
  5. **为什么解决完必须搜一遍冲突标记？**
     因为**Git 不会检查标记有没有删干净**。只要 `git add` 了，它就认为你解决了，
     残留的 `>>>>>>>` 会被当成普通文本提交进去 —— 文件带着垃圾内容，而且没人提示你。
     所以提交前必须自己搜 `<<<<<<<` 确认。这个坑指导书专门提示过。
